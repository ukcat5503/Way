...
public class ParticleChild : MonoBehaviour {
	...
	void Update () {
		if(++frame >= 30){
			...
			if(frame >= destroyFrame / 1.5f){
				transform.position = Vector3.Lerp(transform.position, PuzzleManager.CoinParticleFlyCoinTarget.transform.position, Time.deltaTime * 2f);
			}
